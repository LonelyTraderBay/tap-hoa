...
- Local DB is **Supabase only** (`project_id=tap-hoa`, DB `tap_hoa` `:55422`); Nest `assertTapHoaLocalIdentity()` refuses foreign `DATABASE_URL` / port `54322` / schema mismatches; `docker-compose.dev.yml` deprecated stub.
